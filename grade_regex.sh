#!/usr/bin/env bash
# =============================================================================
# grade_regex.sh  --  Autograder for "Regular Expressions on a Firewall Log"
# Total: 4.00 points.  Clones each student's Codeberg repo, runs the commands
# from their answers.md against YOUR reference firewall.log, scores the rubric
# and PRINTS THE DEDUCTIONS (why each point was lost).
#
# Usage:
#   ./grade_regex.sh [options] <student> [<student> ...]
#   ./grade_regex.sh --csv grades.csv -f students.txt
#
# A <student> can be:
#   - a full repo URL      https://codeberg.org/JeaneP24/regex-firewall
#   - user/repo            JeaneP24/regex-firewall
#   - just a username      JeaneP24            (-> codeberg.org/<user>/regex-firewall)
#   - a local directory    ./repos/jeane
#
# Options:
#   --ref FILE        reference firewall.log (default: ./firewall.log next to script)
#   --csv FILE        append one consolidated row per student to FILE
#   --reponame NAME   repo name to clone for bare usernames (default: regex-firewall)
#   -f, --file FILE   read students (one per line) from FILE
#
...
REPONAME="regex-firewall"; DATA_NAME="firewall.log"
REF=""; CSV=""; STUDENTS=()
while [ $# -gt 0 ]; do
  case "$1" in
    --ref)      REF="$2"; shift 2;;
...
E1=$(grep -cE '^[0-9]' "$REF")
E2=$(grep -cE ' (DROP|REJECT) ' "$REF")
E3=$(grep -cE ' 11\.' "$REF")
E4=$(grep -cE '[0-9]{7}$' "$REF")
E5=$(grep -E '^[0-9]' "$REF" | sed -E 's/^([0-9-]+) [0-9:]+ ([A-Z]+) ([A-Z]+) .*/\1 \2 \3/' | head -5)
E6=$(grep -cE ' ACCEPT TCP .* 80 [0-9]+$' "$REF")
E7=$(grep -cE '^[0-9-]+ 0[0-2]:' "$REF")
EXP=( "" "$E1" "$E2" "$E3" "$E4" "$E5" "$E6" "$E7" )
MULTILINE=" 5 "
HINT=( "" \
 "exclude the # header with a ^ anchor" \
 "match DROP or REJECT as a whole field" \
 "escape the dot and anchor to the src-ip field" \
 "exactly 7 digits anchored at end of line" \
 "rebuild 'date action protocol' with sed capture groups" \
 "combine ACCEPT + TCP + dst-port 80, anchored before size" \
 "match hours 00-02 right after the date" )
...
extract_cmd(){ # $1=task -> student's command for that task (in $ANS)
  [ -f "$ANS" ] || return 0
  awk -v t="$1" '
...
resolve(){ # $1=student -> sets REPO, SRC ; returns 1 on clone failure
  local a="$1" url t
...
grade_one(){
  ...
  for t in 1 2 3 4 5 6 7; do
    ...
    if [ -n "$cmd" ]; then
      ...
      if [ -n "$e" ] && [ "$g" = "$e" ]; then
        ...
        pts=0.20
        if [[ "$MULTILINE" == *" $t "* ]]; then
          DED+=("Task $t: -0.20 - output does not match (${HINT[$t]}).")
        else
          DED+=("Task $t: -0.20 - your command returned '${g:-<empty>}' but the expected answer is '$e' (${HINT[$t]}).")
        fi
      fi
    else
      if [[ "$MULTILINE" != *" $t "* ]] && get_block "$t" | grep -qF "$exp" 2>/dev/null; then
  ...
  # 3) repository & commits (0.40)   [no literal 'task-N' required]
  local nc=0 hasR=0 hasA=0 hasD=0
  [ -f "$REPO/README.md" ] && hasR=1
  [ -f "$ANS" ] && hasA=1
  [ -f "$REPO/$DATA_NAME" ] && hasD=1
  if git -C "$REPO" rev-parse --git-dir >/dev/null 2>&1; then nc=$(git -C "$REPO" rev-list --count HEAD 2>/dev/null || echo 0); fi
  local reppts; reppts=$(awk -v n="$nc" -v r="$hasR" -v a="$hasA" -v d="$hasD" 'BEGIN{
      c = (n>=7?0.20:(n>=4?0.13:(n>=2?0.07:0)))
      p = c + 0.10*r + 0.05*a + 0.05*d; if(p>0.40)p=0.40; printf "%.2f",p }')
  ...
  if [ "$(awk -v p="$reppts" 'BEGIN{print (p<0.40)?1:0}')" = 1 ]; then
    ...
    [ "$hasA" = 0 ] && rr="$rr answers.md not committed;"
    [ "$hasD" = 0 ] && rr="$rr $DATA_NAME not committed;"
    [ "$nc" -lt 7 ] && rr="$rr only $nc commit(s) (one per task ~7 expected, incremental);"
    DED+=("$rr")
  fi
  TOTAL=$(add "$TOTAL" "$reppts")

  # 4) regex technique (0.20)
  local allcmd t5cmd tech=0.00 tnote=""
  allcmd="$(for t in 1 2 3 4 5 6 7; do extract_cmd "$t"; done)"; t5cmd="$(extract_cmd 5)"
  if grep -qE '\bsed\b' <<<"$t5cmd"; then tech=$(add "$tech" 0.10); else tnote="$tnote Task 5 should use sed;"; fi
  if grep -qE 'grep[[:space:]]+-[a-zA-Z]*E|[][{}^$|]|\\\.' <<<"$allcmd"; then tech=$(add "$tech" 0.10)
  else tnote="$tnote commands barely use regex metacharacters (looks brittle);"; fi
  ROWS+=("Regex technique|$tech|0.20")
  [ -n "$tnote" ] && DED+=("Regex technique: -$(sub2 0.20 "$tech") -$tnote")
